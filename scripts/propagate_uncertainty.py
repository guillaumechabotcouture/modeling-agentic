...
def load_outcome_fn(run_dir: str, spec: Optional[str]) -> Callable[[dict], dict]:
    ...
    module_spec = importlib.util.spec_from_file_location("_outcome_mod", path)
    if module_spec is None or module_spec.loader is None:
        raise ImportError(f"could not load {path}")
    module = importlib.util.module_from_spec(module_spec)
    # Add the outcome_fn's directory to sys.path so it can import siblings.
...
def propagate(run_dir: str, n_draws: int = 200, seed: int = 42,
    ...
    # Run one draw at a time, collect results.
    scalar_results: dict[str, list[float]] = {}
    categorical_results: dict[str, list[str]] = {}
    errors: list[dict] = []
    for i in range(n_draws):
        params_for_draw = {name: per_param_samples[name][i]
                           for name in per_param_samples}
        try:
            out = outcome_fn(params_for_draw)
            ...
            if len(errors) > 10 and len(errors) / (i + 1) > 0.2:
                raise RuntimeError(
                    f"outcome_fn raised on >{20}% of draws "
                    f"({len(errors)}/{i+1}). Last error: "
                    f"{errors[-1]}. Aborting."
                )
            continue
        ...
        if not isinstance(out, dict):
            raise TypeError(
                f"outcome_fn must return dict, got {type(out).__name__} on draw {i}"
            )
