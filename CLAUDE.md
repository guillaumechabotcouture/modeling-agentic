...
| 17    | γ, α, β, δ | **Adversarial restructure.** Targets the three error classes that survived ACCEPT in 2026-04-29_161312: (1) paraphrase/label drift across files, (2) self-contradicting artifacts after patches, (3) real-world plausibility (procurement, supply, vintage). γ: `.claude/orchestration/expert_priors.yaml` — 10-prior cross-run institutional memory (Nigeria GBD 12.8M DALYs, global PBO LLIN supply ceiling, GC7 disaggregation, MAP API auth, OR-to-RR Zhang-Yu, multiplicative-independence pitfall, etc.); new `scripts/lib/expert_priors.py` loader with question-keyword + decision-year matchers. α: new `agents/critique_premortem.py` agent + `pre-mortem-domain` skill — clean-slate adversarial critic spawned in PRE-MODEL window (STAGE 3, parallel to identifiability_a_priori). Reads only plan.md + hypotheses.md + matching priors; emits `pre_mortem.yaml` with HIGH-impact concerns categorized ARCHITECTURE/DATA/FEASIBILITY/BLIND_SPOT/EXPERT_PRIOR. Modeler must address each HIGH in `modeling_strategy.md § Pre-mortem Responses` (or scope-declare). 6 validator self-tests (PM1-PM6). β: new `scripts/coherence_audit.py` — three duties (label coherence, cross-file count/cost, self-contradicting artifacts) reusing `_scan_*` primitives from `numeric_consistency.py`. Runs alongside `writer_qa.py` post-WRITE. Validator integration via `_check_coherence_audit` (5 self-tests CA1-CA6) with Phase 10 ψ-style MEDIUM consolidation per duty. Retro on 2026-04-29_161312 fires 8 HIGH label_coherence (R-019), 1 HIGH cross_file_counts (R-005), 1 MEDIUM self_contradicting (M-010). δ: priors registry rides on **literature consensus**, not reviewer judgment. Schema upgrade adds `literature_corroboration: list[Citation]` (≥2 for MEDIUM, ≥3 for HIGH; primary/corroborating/supporting/contextual relevance) and `last_literature_check` per prior. New `auto_validation:` config block (per-severity min sources + max age). New `validate_priors()` (structural — no network) emits issues for insufficient_sources / stale / no_primary_source / unverified_doi; warnings auto-flow into pre-mortem agent spawn prompt via `validation_warnings_for_pre_mortem()`. New `retro_test(run_dir)` walks past critique markdowns and reports coverage of expert-prior-class blockers. New CLI `--validate`, `--match-yaml` (YAML with full corroboration for spawn injection), `--retro-test`, `--enrich-suggestions`. 10 priors re-corroborated with 2-3 cited sources each. Retro-test on 2026-04-29_161312: **87.5% coverage on expert-prior-class blockers** (caught R-007/R-009/R-010/R-011/R-012/R-014/R-018); on 2026-04-28_224202: 100% (D-008). ε: PR #9 review fixes (4e57393 + 4e8f859) — title-case label-coherence regex (added `re.IGNORECASE`); `_read_allocation_csv` searches subdirs; cost-column tries both `annual_cost` and `cost_usd`; `coherence_audit_not_run` registered in escalation thresholds; bare-integer regression fix on no-suffix dollar amounts. |
| 18    | α, β, γ | **Claims ledger.** Structural fix for the paraphrase-drift class (R-019/R-020/$197M conflation): writer cannot re-narrate a number that doesn't exist in the ledger. α: analyst produces `models/claims_ledger.yaml` listing every quantitative or categorical claim with id/kind/value/units/CI/source/causal_label. Writer rebound to `[CLAIM:claim_id]` references for every number, percentage, count, currency figure, and verdict label. New `scripts/render_claims.py` substitutes references at write-time (8 self-tests: per-kind formatters + CI rendering + ledger validation + run-level render). New STAGE 8.1 RENDER_CLAIM_REFERENCES block in lead prompt. New `claims_ledger` artifact in `.claude/orchestration/rigor_artifacts.yaml` (stage `model`, `scope_declarable: false`). New `_check_claims_ledger_present` validator (6 self-tests CL1-CL6: round gating, missing/MEDIUM-vs-HIGH escalation, schema validity, CI bounds, empty-ledger). β: new `ledger_binding` duty in `coherence_audit.py` — scans report.md for currency/verdict claims and verifies each appears in the ledger's value renderings (incl. M/K/B suffixes, comma forms, CI endpoints). HIGH on every unbound prose number/label. Generic-use qualifiers preserved (e.g., "sensitivity analysis" heading doesn't fire SENSITIVE). 5 new self-tests (T7-T11). Redteam Duty 1 simplified: validates ledger internal consistency (CI semantics, source_field resolution, causal_label honesty) instead of manual cross-file numeric audit (now done by auditor). γ: pre-mortem prompt tightening — must preserve question-stated frameworks when suggesting fallbacks. Phase 17 retro on 2026-05-03_115318 round 1 was RETHINK_STRUCTURAL because PM-003 ("fall back to compartmental Standard model") was read as "drop Starsim entirely"; γ adds a worked-example contrast in `agents/critique_premortem.py` showing right vs wrong phrasing. |
| 19    | α, β, γ, δ | **Effort-anchored rigor.** Targets the under-investment class: previous gates verified artifact *existence and consistency*, not whether the underlying work was done with enough effort. A modeler could ship `uncertainty_report.yaml` with `n_draws=50` and pass. α: new `.claude/orchestration/effort_floors.yaml` manifest declaring quantitative minimums (200 local UQ draws, 1000 cloud, 3 calibration restarts, 500 iterations, 3 sensitivity perturbation points, 5 perturbed params, held-out fold required, zero shortcut markers). New `scripts/effort_floors.py` with four probes (`yaml_field`, `yaml_field_list_length`, `yaml_field_truthy`, `ast_shortcut_scan`) and 13 self-tests. AST probe walks `models/*.py` for TODO/FIXME/HACK comments, bare `except: pass`, and small literals on shortcut-named kwargs/assignments (`n_replicates=1`, `n_draws=5`, `maxiter=10`). New `_check_effort_floors` validator: drafting window at round 1 demotes everything to MEDIUM; round ≥ 2 honors manifest severities; ≥ 3 shortcut findings escalate to HIGH `effort_floors_many_shortcuts` regardless of manifest. HIGH floors with `scope_declarable: false` are explicitly NOT silenceable. 8 self-tests EF1-EF8. β: scope-declaration budget cap. New `_check_scope_declaration_budget` in validator. Default cap 3; modeler may raise to hard ceiling 5 via top-level `budget: {cap: N, overage_justification: "..."}` block (non-empty justification required). Empty entries don't burn budget but emit a consolidated MEDIUM advisory. 9 self-tests SB1-SB9. γ: benchmark-locked acceptance. New `models/benchmark_targets.yaml` artifact (manifest entry, `scope_declarable: true` because some questions imply novel populations). New `benchmark-locked-acceptance` skill teaches the planner-then-modeler workflow. New `scripts/benchmark_match.py` with `--self-test`: per-target schema (`target_value`, `tolerance_factor` and/or `tolerance_abs`, `source`, `computed_value` inline OR `computed_field: "path.yaml::dot.key"`); intersection of factor + abs bands; verdicts PASS / DRIFT / MALFORMED / ABSENT. 9 self-tests T1-T9. New `_check_benchmark_match` validator: targets absent at r ≥ 2 → MEDIUM `benchmark_targets_missing`; drift → HIGH `benchmark_drift` per target (cap 5 + advisory); missing computed → HIGH `benchmark_computed_missing`. 9 self-tests BM1-BM9. δ: sufficiency critic. New `agents/critique_sufficiency.py` + `evidence-sufficiency` skill. Post-WRITE adversarial critic spawned in parallel with writer-QA and coherence audit. Reads `report.md` + `claims_ledger.yaml` + `benchmark_match.yaml` + `effort_floors_report.yaml` + (if present) `models/calibration_result.yaml` + `uncertainty_report.yaml`. Emits per-claim OVERCLAIMED / ADEQUATE / UNDERCLAIMED verdicts with quoted phrases, evidence pointers, and suggested restatements. Decision rules: point-estimate causal claim with `n_restarts < 3` or no held-out → OVERCLAIMED; 95% CI on model output with `n_draws < 200` (local) or `< 1000` (cloud) → OVERCLAIMED; benchmark-anchored claim with DRIFT/missing_computed → OVERCLAIMED. Registered in `agents/__init__.py` with maxTurns=30 (Opus, no WebSearch — pure artifact reading). New `_check_sufficiency_critic` validator: drafting window at r=1; OVERCLAIMED at r ≥ 2 fires HIGH `claim_overclaimed` per verdict (cap 5 + advisory), scope-declarable. 8 self-tests SF1-SF8. **Net effect**: the harness now distinguishes "the optimizer ran" from "calibration was attempted with effort" (α), bounds the "scope-declare everything" shortcut (β), anchors fitted outputs to literature (γ), and asks whether each claim's strength matches its evidence base (δ). |
...
```bash
# All sixteen self-tests must report "OK: all self-test cases passed."
python3 scripts/benchmark_match.py --self-test             # Phase 19 γ
python3 scripts/coherence_audit.py --self-test             # Phase 17 β
python3 scripts/effort_floors.py --self-test               # Phase 19 α
...
python3 scripts/render_claims.py --self-test               # Phase 18 α
